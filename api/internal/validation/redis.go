...
func ValidateRedisConnection(logger *zap.Logger, cfg *config.Config) (*RedisConnectionConfig, error) {
	...
	logger.Info("Redis connection validated",
		zap.String("host", logging.SanitizeForLogging(logger, "host", redisHost, logging.SanitizeConfig{
		...
		zap.Int("db", redisDB),
		zap.String("address", logging.SanitizeForLogging(logger, "address", redisConfig.Address, logging.SanitizeConfig{
			RedactPasswords: true,
			RedactHosts:     true,
			MaxStringLength: 50,
		})))
