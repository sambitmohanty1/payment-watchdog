...
function validateRequest(request: NextRequest): { isValid: boolean; error?: string } {
  ...
  // Company ID is required for security
  if (!companyId) {
    return { isValid: false, error: 'company_id query parameter is required for security' };
  }
...
export async function GET(request: NextRequest) {
  try {
    // Security validation - company_id is required
    ...
    const backendUrl = `${BACKEND_URL}/api/v1/dashboard/stats${queryString}`;
