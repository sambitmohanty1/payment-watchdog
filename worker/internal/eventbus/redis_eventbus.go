...
func (r *RedisEventBus) handleMessage(sub *RedisSubscription, msg redis.XMessage) error {
	...
	return sub.handler.Handle(sub.ctx, eventData)
}
...
func (r *RedisEventBus) GetHealthStatus() *interfaces.EventBusStatus {
	return &interfaces.EventBusStatus{
		IsConnected:     r.client.Ping(r.ctx).Err() == nil,
		SubscriberCount: len(r.subscribers),
		MessageCount:    0, // Would need to track this
		ErrorCount:      0, // Would need to track this
	}
}

// Publish implements event publishing to Redis
func (r *RedisEventBus) Publish(ctx context.Context, topic string, event interface{}) error {
	...
	// Publish to Redis stream or pub/sub based on event type
	// For now, using a simple pub/sub approach
	payload := map[string]interface{}{
		"event": string(eventData),
		"topic": topic,
	}

	cmd := r.client.Publish(ctx, topic, payload)
	err = cmd.Err()
	if err != nil {
